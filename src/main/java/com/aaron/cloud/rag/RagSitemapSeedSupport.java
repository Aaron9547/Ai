...
public final class RagSitemapSeedSupport {
            ...
            try {
                Document doc =
                        Jsoup.connect(smUrl)
                                ...
                                .get();
                collectLoc(doc, smUrl, rootDomain, maxUrls, out, 3);
    ...
    private static void collectLoc(
            Document doc, String baseForRelative, String rootDomain, int maxUrls, List<String> out, int depthLeft) {
        if (depthLeft <= 0) {
            return;
        }
                ...
                try {
                    Document child =
                            Jsoup.connect(abs)
                                    ...
                                    .get();
                    collectLoc(child, abs, rootDomain, maxUrls, out, depthLeft - 1);
                } catch (Exception ignored) {
                    // skip child sitemap
                }
