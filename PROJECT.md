...
- **固定源出站代理（进程）**：**`WebSearchFixedSourceHttpProperties`**（**`application-local.yml`** / 环境变量 / JVM）；**`use-system-proxy=true`** 映射 **127.0.0.1:7890 HTTP**（避免 JVM SOCKS）。**`application.yml` 不再内置默认项**。
- **百度新闻固定源修复**：**`BaiduNewsHtmlWebSearchProvider`** 强制直连、识别安全验证页并 WARN；空结果不再写入进程内短缓存。**无 DB migrate**。
