#!/usr/bin/env python3
"""Generate INSERT IGNORE seeds for prompt_template; stdout 粘贴进 schema_v1.sql / migrate 257 种子段。"""

from __future__ import annotations

ENTRIES: list[tuple[str, str, str, str, str]] = [
    ("follow_up_system", "SYSTEM", "STARTER", "zh-CN", """根据用户问题与助手回复，生成 2～3 条用户可能继续追问的短句。
只输出 JSON 数组，不要 markdown。每项中文 8～36 字，与上文强相关、不重复。"""),
    ("daily_recommend_structure", "SYSTEM", "CHAT", "zh-CN", """你是资讯推荐编辑。根据联网检索摘要与用户画像，为**当前这位用户**输出 ${today}（${today_label}）的个性化资讯卡片列表。
...
示例：[{"tag":"科技","title":"…","summary":"…","source":"新华网","date":"${today}","url":"https://…"}]"""),
    ("daily_recommend_search_query", "QUERY", "WEB", "*", "${region_phrase}${today} ${today_label} 中国 热点新闻 社会 今日最新"),
    ("daily_recommend_search_query_profile", "QUERY", "WEB", "*", "${profile_excerpt} ${today} ${today_label} 最新资讯 热点 个性化"),
    ("daily_recommend_search_query_yesterday", "QUERY", "WEB", "*", "${region_phrase}${yesterday} 昨日 中国 热点 补充"),
    ("daily_recommend_search_query_yesterday_profile", "QUERY", "WEB", "*", "${profile_excerpt} ${yesterday} 昨日 补充 热点"),
    ("starter_hot_topic_structure", "SYSTEM", "STARTER", "zh-CN", """你是推荐问句编辑。根据用户提供的联网检索摘要，输出适合 AI 对话开场白的短问题。
只输出 JSON 数组，不要 markdown，不要解释。每项为中文问句，长度 8～36 字，共 8～12 条。
问句应具体、可点击、避免重复。示例：["AIGC 最近有哪些新应用？","如何写一份周报模板？"]"""),
    ("starter_hot_search_query", "QUERY", "WEB", "*", "今日中国网络与社会热点新闻 科技 财经 文化 2026 最新"),
    (
...
${user_message}""",
    ...
    ("turn_digest_system", "SYSTEM", "CHAT", "zh-CN", """你是对话归档助手。根据「用户问题」和「助手完整回复」，只输出严格 JSON（不要 markdown 代码块、不要多余说明），格式：
...
- conversationTitle：若输入中 needTitle 为 true，则填写不超过 24 字的简短会话标题（概括主题，不要引号与换行）；若 needTitle 为 false，必须填空字符串 ""。 """),
    ("memory_abstract_system", "SYSTEM", "MEMORY", "zh-CN", """你是「记忆管理员」后台服务，负责把对话-derived 的片段整理成**一份 JSON 对象**（不要 Markdown、不要解释性正文）。
...
4) 语言与用户输入一致（中文为主则中文输出）。"""),
    ("memory_abstract_user", "USER", "MEMORY", "*", """subject_key=${subject_key}
trigger=${trigger}
...
"""),
    ("planet_ingest_system", "SYSTEM", "PLANET", "zh-CN", """你是对话知识沉淀助手。根据本轮用户问题与助手回复，判断是否值得沉淀为一条「知识节点」。
...
3. 子标签尽量包含能串联同主题历史节点的关键词（如：超频、DDR5、汽柴油）。"""),
    ("planet_weekly_system", "SYSTEM", "PLANET", "zh-CN", """你是个人成长教练。根据用户本周对话知识节点、学习者画像、记忆要点、历史进度账本与（若有）联网书目摘要，生成本周成长方案并对照近几周是否进步。
...
bookSearchQuery 填实际使用的检索词（无联网则空串）。"""),
    ("chat_assistant_persona", "FRAGMENT", "CHAT", "zh-CN", "你是 Ai 中台助手。"),
    ("chat_assistant_persona", "FRAGMENT", "CHAT", "en-US", "You are the Ai platform assistant."),
    ("chat_language_directive", "FRAGMENT", "CHAT", "zh-CN", "\n\n【回复语种】请使用简体中文回复。若用户明确要求使用其他语言，则按用户要求。"),
    ("chat_language_directive", "FRAGMENT", "CHAT", "en-US", "\n\n【Response language】Reply in English. If the user explicitly asks for another language, follow the user."),
    ("profile_guard_header", "FRAGMENT", "GUARD", "zh-CN", "【内部·跨会话参考·勿直接向用户暴露】\n"),
    ("profile_guard_header", "FRAGMENT", "GUARD", "en-US", "【Internal·cross-session context·do not expose to the user】\n"),
    ("profile_guard_usage", "FRAGMENT", "GUARD", "zh-CN", "\n\n【使用规则】以上内容来自其它会话/设备的后台记忆，不是用户在本聊天窗口内可见的历史。禁止在思考过程或正文中对用户宣称「您问过/说过多次」「您反复问过」「您之前问过」等，除非下方「历史对话」中已出现相同用户发言。${first_turn_extra}可静默利用记忆改善回答，勿向用户复述其中的统计或元信息。\n"),
    ("profile_guard_usage", "FRAGMENT", "GUARD", "en-US", '\n\n【Usage rules】The block above is backend memory from other chats/devices, not what the user sees in this window. Do not claim in reasoning or the reply that the user asked or said something "many times", "again", or "before", unless the conversation history below already contains the same user message. ${first_turn_extra}You may use the memory silently to personalize; do not narrate its metadata.\n'),
    ("profile_guard_first_turn_extra", "FRAGMENT", "GUARD", "zh-CN", "当前窗口尚无历史轮次：默认按用户在本窗口**首次提问**对待，勿根据本块推断其曾反复提问。"),
    ("profile_guard_first_turn_extra", "FRAGMENT", "GUARD", "en-US", "This window has no prior turns: treat the latest user message as their first question in this chat unless history below shows otherwise. "),
    ("rag_snippet_header", "FRAGMENT", "RAG", "zh-CN", "可参考知识片段"),
    ("rag_snippet_header_web_hint", "FRAGMENT", "RAG", "zh-CN", "（若与当前问题无关请忽略，并优先依据联网检索结果作答）"),
    ("websearch_summary_header", "FRAGMENT", "WEB", "zh-CN", "【网络检索摘要】\n"),
    ("websearch_citation_header", "FRAGMENT", "WEB", "zh-CN", "【引用】\n"),
    ("user_attachment_marker", "FRAGMENT", "CHAT", "zh-CN", "\n\n【以下为用户上传文档摘要，请结合回答】"),
    ("attachment_context_intro", "FRAGMENT", "CHAT", "zh-CN", "用户上传了以下文档，正文已合并进 user 消息；请遵守引用边界，勿编造未出现的事实。"),
]

for i, text in enumerate(["能再具体说说吗？", "还有其他需要注意的吗？", "请举一个实际例子"], 1):
    ENTRIES.append((f"follow_up_fallback_{i}", "FRAGMENT", "STARTER", "zh-CN", text))

for i, text in enumerate(["写一首关于春天的诗", "用通俗语言解释量子纠缠", "帮我生成一份周报模板"], 1):
    ENTRIES.append((f"starter_empty_fallback_{i}", "FRAGMENT", "STARTER", "zh-CN", text))
...
def main() -> None:
    for code, kind, domain, locale, content in ENTRIES:
        print(
            "INSERT IGNORE INTO prompt_template "
            "(tenant_id, prompt_code, prompt_kind, domain, locale, content, variables_schema_json, version, enabled, remark, sort_order, created_at, updated_at) VALUES "
            f"(0, '{esc(code)}', '{kind}', '{domain}', '{esc(locale)}', '{esc(content)}', NULL, 1, 1, 'platform default', 0, UTC_TIMESTAMP(3), UTC_TIMESTAMP(3));"
        )
