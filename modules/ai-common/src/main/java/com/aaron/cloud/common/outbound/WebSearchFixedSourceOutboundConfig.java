...
/** 固定联网源 / MCP 等出站：直连 / 显式 HTTP·SOCKS / JVM 代理属性 / 本地 Clash HTTP。 */
public final class WebSearchFixedSourceOutboundConfig {
  ...
  private static final int DIAG_TIMEOUT_MS = 10_000;
  ...
  public static WebSearchFixedSourceOutboundConfig resolveForTenant(
      WebSearchFixedSourceOutboundRuntime tenant, WebSearchFixedSourceOutboundConfig globalFallback) {
    if (tenant != null && tenant.enabled() && tenant.host() != null && !tenant.host().isBlank()) {
      return explicit(tenant.host().trim(), tenant.port(), tenant.normalizedType());
    }
    return globalFallback == null ? direct() : globalFallback;
  }

  public static WebSearchFixedSourceOutboundConfig resolve(
    ...
    if (jvm != null && jvm.type() != Proxy.Type.DIRECT) {
      if (jvm.type() == Proxy.Type.SOCKS && isLoopback(jvm)) {
        return localClashHttpExplicit(null, null, "jvmSocksRemappedToHttp");
      }
  ...
  private static WebSearchFixedSourceOutboundConfig localClashHttpExplicit(
    ...
    return new WebSearchFixedSourceOutboundConfig(
        cfg.mode(), cfg.proxy(), cfg.label() + "(" + reason + ")");
  }
  ...
  private static WebSearchFixedSourceOutboundConfig explicit(String host, int port, String type) {
    Proxy.Type proxyType =
        type != null && type.trim().equalsIgnoreCase("SOCKS") ? Proxy.Type.SOCKS : Proxy.Type.HTTP;
  ...
  public static WebSearchFixedSourceOutboundConfig direct() {
    return new WebSearchFixedSourceOutboundConfig(Mode.DIRECT, Proxy.NO_PROXY, "NO_PROXY");
  }

  public static String diagnostics(WebSearchFixedSourceOutboundConfig config) {
    List<String> parts = new ArrayList<>();
    parts.add("fixedSourceMode=" + config.label());
    parts.add("timeoutMs=" + DIAG_TIMEOUT_MS);
    ...
    if (config.mode() == Mode.DIRECT) {
      parts.add(
          "hint=境外出站需代理时可配置 ai.websearch.fixed.proxy-host=127.0.0.1 proxy-port=7890（Clash 常见；MCP 共用）");
    }
